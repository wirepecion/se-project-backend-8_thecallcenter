...
exports.getBookings = async (req, res, next) => {
    ...
    try {
        ...
        res.status(500).json({
            success: false,
            message: 'Cannot find any bookings'
        });
    }
};
...
exports.addBooking = async (req, res, next) => {
    try {
        ...
        const existedHotel = await Hotel.find({user:req.user.id});

        //If the user is not an admin, he/she can only book up to 3 nights
        if (existedHotel.length >= 3 && req.user.role !== 'admin') {
            return res.status(400).json({
                success: false,
